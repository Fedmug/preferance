...
func VisitBoundedCompositions(n int8, bound []int8, visitor func(composition)) {
	var sum int8
	for _, b := range bound {
		sum += b
	}
...
func isOrdered(comp composition, excludeFirstElement bool) bool {
...
func SuitSizesCounter(nCards, nSuits, maxSuitSize int8, trump bool) int {
	...
	VisitBoundedCompositions(nCards, suitBounds, func(suitSizes composition) {
